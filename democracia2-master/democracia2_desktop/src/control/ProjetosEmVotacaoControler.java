...
public class ProjetosEmVotacaoControler implements Initializable {
  ...
  public void initialize(URL arg0, ResourceBundle arg1) {
    ...
    try {
      URL url = new URL("http://localhost:8080/api/votacoes-em-curso");
      ...
      Gson gson = new Gson();
      ProjetoDeLeiDTO[] projetoDeLeiArray = gson.fromJson(sb.toString(), ProjetoDeLeiDTO[].class);
      projetos = Arrays.asList(projetoDeLeiArray);
      ObservableList<ProjetoDeLeiDTO> projetosParaTabela =
          FXCollections.observableArrayList(projetos);
      table.setItems(projetosParaTabela);
  ...
  public void goToProj(ActionEvent event) throws IOException {
    ...
    if (found) {
      ...
      FXMLLoader loader = new FXMLLoader(getClass().getResource("/paginas/votar.fxml"));
      root = loader.load();
      VotarControler votarControler = loader.getController();
      votarControler.setId(Long.parseLong(textField.getText()));
      stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
      scene = new Scene(root);
      stage.setScene(scene);
      stage.show();
